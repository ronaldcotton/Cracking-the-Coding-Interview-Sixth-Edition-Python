...
if __name__ == '__main__':
	...
	answer = ll.sum_lists_reverse_order()
	
	print('Add two numbers together:')
	print(answer)
	
	ll2 = LinkedList() # create Linked List
	ll2.head = Node('6')
	ll2.head.next = Node('1')
	(ll2.head.next).next = Node('7')
	((ll2.head.next).next).next = Node('2')
	(((ll2.head.next).next).next).next = Node('9')
	((((ll2.head.next).next).next).next).next = Node('5')
	print('Linked List:')
	print(ll2)
	
	answer = ll2.sum_lists()
