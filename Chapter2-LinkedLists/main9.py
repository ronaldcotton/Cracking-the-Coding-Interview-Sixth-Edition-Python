...
# plus Interview Question 2.7 - Loop Detection
# Python Example
...
class LinkedList(object):
	...
	# returns None if no loop - returns data value if loop
	def loop_detection(self):
		single = runner = self.head
		string_array = []
		while runner is not None: # initial is going to move two at a time
			single = single.next # when complete, middle will be in middle
			if runner.next is None:
				runner = runner.next
				return None
			else:
				runner = (runner.next).next
			if single.data == runner.data and single.next == runner.next:
				return single.data
...
if __name__ == '__main__':
	...
	print(f'Loop? {str(ll.loop_detection())}')
